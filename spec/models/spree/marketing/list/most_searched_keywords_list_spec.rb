...
describe Spree::Marketing::List::MostSearchedKeywords, type: :model do
  ...
  it_behaves_like 'acts_as_multilist', Spree::Marketing::List::MostSearchedKeywords

  describe 'Constants' do
    it 'NAME_TEXT equals to name representation for list' do
      expect(Spree::Marketing::List::MostSearchedKeywords::NAME_TEXT).to eq 'Most Searched Keywords'
    end
    it 'ENTITY_KEY equals to entity attribute for list' do
      expect(Spree::Marketing::List::MostSearchedKeywords::ENTITY_KEY).to eq 'searched_keyword'
    end
    it 'ENTITY_TYPE equals to type of entity for list' do
      expect(Spree::Marketing::List::MostSearchedKeywords::ENTITY_TYPE).to be_nil
    end
    it 'TIME_FRAME equals to time frame used in filtering of users for list' do
      expect(Spree::Marketing::List::MostSearchedKeywords::TIME_FRAME).to eq 1.month
    end
    it 'MOST_SEARCHED_KEYWORD_COUNT equals to count of keywords to be used for data for lists' do
      expect(Spree::Marketing::List::MostSearchedKeywords::MOST_SEARCHRD_KEYWORD_COUNT).to eq 5
    end
    it 'AVAILABLE_REPORTS equals to array of reports for this list type' do
      expect(Spree::Marketing::List::MostSearchedKeywords::AVAILABLE_REPORTS).to eq %i[cart_additions_by purchases_by]
    end
  end
  ...
  describe 'Methods' do
    describe '#user_ids' do
      context 'when search events were created before TIME_FRAME' do
        ...
        it 'includes users with search event for the entity keyword' do
          expect(Spree::Marketing::List::MostSearchedKeywords.new(searched_keyword: searched_keyword).user_ids).to include user_with_search_event_for_searched_keyword.id
        end
        it "doesn't include users with search event for the entity keyword created before time frame" do
          expect(Spree::Marketing::List::MostSearchedKeywords.new(searched_keyword: searched_keyword).user_ids).to_not include user_with_old_search_event.id
        end
      end
      ...
      context 'when user is not registered' do
        ...
        it "doesn't include guest user page events" do
          expect(Spree::Marketing::List::MostSearchedKeywords.new(searched_keyword: searched_keyword).user_ids).to_not include nil
        end
      end
      ...
      context 'with users having search events of entity keyword and other keywords' do
        ...
        it "doesn't includes users having only search events for keywords other than the entity keyword" do
          expect(Spree::Marketing::List::MostSearchedKeywords.new(searched_keyword: searched_keyword).user_ids).to_not include user_with_search_event_for_another_searched_keyword.id
        end
      end
    end
    ...
    describe '.data' do
      context 'method flow' do
        it 'includes searched keyword' do
          expect(Spree::Marketing::List::MostSearchedKeywords.send(:data)).to include searched_keyword
        end
      end
      ...
      context 'limit to MOST_SEARCHRD_KEYWORD_COUNT' do
        ...
        it 'only includes 5 keywords which are most searched' do
          expect(Spree::Marketing::List::MostSearchedKeywords.send(:data)).to include searched_keyword, second_searched_keyword, third_searched_keyword, fourth_searched_keyword, fifth_searched_keyword
        end
        it "doesn't keyword which is not in top 5 most searched" do
          expect(Spree::Marketing::List::MostSearchedKeywords.send(:data)).to_not include sixth_searched_keyword
        end
      end
      ...
      context 'with old search events of keywords' do
        ...
        it 'returns keywords which will not include keyword having searh events only before time frame' do
          expect(Spree::Marketing::List::MostSearchedKeywords.send(:data)).to_not include other_searched_keyword
        end
      end
    end
    ...
    context '#entity_name' do
      it 'returns the searched keyword' do
        expect(Spree::Marketing::List::MostSearchedKeywords.new(searched_keyword: searched_keyword).send(:entity_name)).to eq entity_name
      end
    end
  end
end
