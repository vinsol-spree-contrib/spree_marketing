    ...
    describe '#user_ids' do
      context 'with orders not having selected state' do
        ...
        it 'include users with orders in the entity zone' do
          expect(Spree::Marketing::LeastZoneWiseOrdersList.new(entity_id: state.id, entity_type: 'Spree::State').user_ids).to include user_with_completed_orders_with_shipping_address_having_given_state.id
        end
        it "doesn't include the users which do not have order in entity state" do
          expect(Spree::Marketing::LeastZoneWiseOrdersList.new(entity_id: state.id, entity_type: 'Spree::State').user_ids).to_not include user_with_order_in_other_state.id
        end
      end
      ...
      context 'when user is not registered' do
        ...
        it "doesn't include guest users who have ordered in the entity state" do
          expect(Spree::Marketing::LeastZoneWiseOrdersList.new(entity_id: state.id, entity_type: 'Spree::State').send :emails).to_not include guest_user_email
        end
      end
      ...
      context 'when orders are completed before TIME_FRAME' do
        ...
        it "doesn't include users who have orders in entity state completed before time frame" do
          expect(Spree::Marketing::LeastZoneWiseOrdersList.new(entity_id: state.id, entity_type: 'Spree::State').send :user_ids).to_not include user_having_old_completed_orders_in_given_state.id
        end
      end
    end
  end
