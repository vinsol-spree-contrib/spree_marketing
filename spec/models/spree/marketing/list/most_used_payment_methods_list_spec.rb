...
describe Spree::Marketing::MostUsedPaymentMethodsList, type: :model do
  ...
  describe 'Constants' do
    it 'NAME_TEXT equals to name representation for list' do
      expect(Spree::Marketing::MostUsedPaymentMethodsList::NAME_TEXT).to eq 'Most Used Payment Methods'
    end
    it 'ENTITY_KEY equals to entity attribute for list' do
      expect(Spree::Marketing::MostUsedPaymentMethodsList::ENTITY_KEY).to eq 'entity_id'
    end
    it 'ENTITY_TYPE equals to type of entity for list' do
      expect(Spree::Marketing::MostUsedPaymentMethodsList::ENTITY_TYPE).to eq 'Spree::PaymentMethod'
    end
    it 'TIME_FRAME equals to time frame used in filtering of users for list' do
      expect(Spree::Marketing::MostUsedPaymentMethodsList::TIME_FRAME).to eq 1.month
    end
    it 'MOST_USED_PAYMENT_METHODS_COUNT equals to count of payment methods to be used for data for lists' do
      expect(Spree::Marketing::MostUsedPaymentMethodsList::MOST_USED_PAYMENT_METHODS_COUNT).to eq 5
    end
  ...
  describe 'methods' do
    describe '#user_ids' do
      context 'with users having greater than 5 orders' do
        ...
        it { expect(Spree::Marketing::MostUsedPaymentMethodsList.new(entity_id: payment_method.id, entity_type: 'Spree::PaymentMethod').user_ids).to include user_with_more_than_5_completed_orders.id }
        it { expect(Spree::Marketing::MostUsedPaymentMethodsList.new(entity_id: payment_method.id, entity_type: 'Spree::PaymentMethod').user_ids).to_not include user_with_less_than_5_completed_orders.id }
      end
      ...
      context 'with users having greater than 5 orders orders with selected payment method' do
        ...
        it { expect(Spree::Marketing::MostUsedPaymentMethodsList.new(entity_id: payment_method.id, entity_type: 'Spree::PaymentMethod').user_ids).to include user_with_more_than_5_completed_orders.id }
        it { expect(Spree::Marketing::MostUsedPaymentMethodsList.new(entity_id: payment_method.id, entity_type: 'Spree::PaymentMethod').user_ids).to_not include user_with_more_than_5_completed_orders_with_other_payment_method.id }
      end
      ...
      context 'with users having greater than 5 orders completed before TIME_FRAME' do
        ...
        it { expect(Spree::Marketing::MostUsedPaymentMethodsList.new(entity_id: payment_method.id, entity_type: 'Spree::PaymentMethod').user_ids).to include user_with_more_than_5_completed_orders.id }
        it { expect(Spree::Marketing::MostUsedPaymentMethodsList.new(entity_id: payment_method.id, entity_type: 'Spree::PaymentMethod').user_ids).to_not include user_having_more_than_5_old_completed_orders.id }
      end
